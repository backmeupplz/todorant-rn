...
struct SegmentedProgressBarView: View {
    ...
    var body: some View {
      HStack {
        ...
        maximumProgress < 15 ? AnyView(SegmentedProgressBarView.SegmentedProgressBar(currentProgress: currentProgress, maximumProgress: maximumProgress)) :
          AnyView(SegmentedProgressBarView.LinearProgressBar(currentProgress: currentProgress, maximumProgress: maximumProgress))
      ...
      var body: some View {
         HStack(spacing: spacing) {
           ForEach(0 ..< maximumProgress) { index in
             Capsule()
               .foregroundColor(index < self.currentProgress ? self.selectedColor : self.unselectedColor)
           }
         }
         .frame(maxHeight: 2)
         .clipShape(Capsule())
    ...
    var body: some View {
      GeometryReader { geometry in
        ZStack(alignment: .leading) {
          ...
          Rectangle().frame(width: CGFloat(Int(geometry.size.width)/maximumProgress*currentProgress))
